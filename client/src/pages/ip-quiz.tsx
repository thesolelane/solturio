...
const CATEGORIES = [
  ...
  { name: "IP Symbols", icon: "®", source: "Multiple" },
  { name: "Copyright", icon: "©️", source: "Copyright Office" },
  { name: "Copyright Law", icon: "📜", source: "Copyright Office" },
  { name: "NFT & Blockchain", icon: "🔗", source: "Multiple" },
  { name: "Patent", icon: "💡", source: "USPTO/EPO" },
  { name: "Trademark", icon: "™️", source: "USPTO" },
  { name: "Trademark Basics", icon: "📝", source: "USPTO" },
  { name: "USPTO Trademarks", icon: "🇺🇸", source: "USPTO" },
  { name: "WIPO Basics", icon: "🌍", source: "WIPO" },
  { name: "EUIPO Rights", icon: "🇪🇺", source: "EUIPO" },
  { name: "EPO Patents", icon: "🛡️", source: "EPO" },
  { name: "International IP", icon: "🌐", source: "WIPO" },
  { name: "General IP", icon: "📚", source: "Multiple" },
  { name: "IP Enforcement", icon: "⚖️", source: "All" },
  { name: "Copyright Global", icon: "🌎", source: "Multiple" },
  { name: "Cryptocurrency IP", icon: "₿", source: "USPTO" },
  { name: "Trade Secrets", icon: "🔒", source: "USPTO" },
];
...
export default function IPQuiz() {
  ...
  const getMockQuestion = (category: string, points: number) => {
    const mockQuestions = [
      {
        ...
        answer: "®",
        explanation:
          "The ® symbol (R in a circle) indicates a federally registered trademark. It can only be used after USPTO registration is complete.",
      },
      {
        ...
        explanation:
          "Under WIPO and most national laws, copyright lasts for the life of the creator plus 70 years after death.",
      },
      {
        ...
        answer: "25 years",
        explanation:
          "EUIPO registered community designs can be protected for up to 25 years with renewals every 5 years.",
      },
    ];
    ...
    return {
      ...
      points,
      ...selected,
      sourceUrl: category.includes("USPTO")
        ? "https://www.uspto.gov"
        : category.includes("WIPO")
          ? "https://www.wipo.int"
          : category.includes("EUIPO")
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            <div>
              ...
              <CardDescription>Test your IP knowledge and earn $SOLT rewards!</CardDescription>
            </div>
      ...
      {!currentQuestion ? (
        <Card>
          ...
          <CardContent>
                  ...
                  <tr>
                    ...
                    {POINT_VALUES.map((points) => (
                      <th key={points} className="text-center p-2">
                        <span className={`font-bold ${getPointColor(points)}`}>{points}</span>
                      </th>
                    ))}
                  </tr>
                </thead>
            ...
            <Alert className="mt-4">
              ...
              <AlertDescription>
                <strong>Scoring:</strong> Answer correctly to earn points. Build streaks for bonus
                $SOLT rewards! All questions cite official USPTO and Copyright.gov sources.
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            {isTimerActive && <Progress value={(timeLeft / 30) * 100} className="mt-2" />}
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            {!showAnswer && (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  {((currentQuestion as any).options || ["A", "B", "C", "D"]).map(
                    (option: string, index: number) => (
                      <Button
                        ...
                        onClick={() =>
                          !eliminatedOptions.includes(option) && setSelectedAnswer(option)
                        }
            ...
            {showAnswer && (
              <div className="space-y-4">
                ...
                <Button onClick={() => setCurrentQuestion(null)} className="w-full">
