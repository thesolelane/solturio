...
export const TOKEN_LAUNCH_FIELDS: FieldWeight[] = [
  ...
  { key: 'launchPlatform', label: 'Launch Platform', weight: 5, required: true, category: 'identity' },
  { key: 'launchTimeline', label: 'Launch Timeline', weight: 5, required: true, category: 'identity' },
  { key: 'projectSummary', label: 'Project Summary', weight: 5, required: true, category: 'identity' },
  { key: 'tokenCategory', label: 'Token Category', weight: 5, required: true, category: 'identity' },
  { key: 'totalSupply', label: 'Total Supply', weight: 5, required: true, category: 'transparency' },
  { key: 'authorityWallet', label: 'Authority Wallet', weight: 8, required: true, category: 'verification' },
  { key: 'twitterHandle', label: 'Twitter/X Handle', weight: 5, required: true, category: 'community' },

  { key: 'proofPostUrl1', label: 'Proof Post 1', weight: 7, required: false, category: 'verification' },
  { key: 'proofPostUrl2', label: 'Proof Post 2', weight: 5, required: false, category: 'verification' },
  { key: 'tokenomicsDetails', label: 'Tokenomics Details', weight: 3, required: false, category: 'transparency' },
  { key: 'supplyLocked', label: 'Supply Locked', weight: 3, required: false, category: 'transparency' },
  { key: 'lockDuration', label: 'Lock Duration', weight: 2, required: false, category: 'transparency' },
  { key: 'additionalWallet1Address', label: 'Additional Wallet 1', weight: 2, required: false, category: 'verification' },
  { key: 'additionalWallet2Address', label: 'Additional Wallet 2', weight: 1, required: false, category: 'verification' },
  ...
  { key: 'githubRepoUrl', label: 'GitHub Repository', weight: 3, required: false, category: 'transparency' },
  { key: 'tokenContractAddress', label: 'Contract Address', weight: 4, required: false, category: 'legal' },
];

export const MAX_SCORE = TOKEN_LAUNCH_FIELDS.reduce((sum, f) => sum + f.weight, 0);
export const REQUIRED_SCORE = TOKEN_LAUNCH_FIELDS.filter(f => f.required).reduce((sum, f) => sum + f.weight, 0);
...
export function calculateRegistrationStrength(registrationData: Record<string, any>): RegistrationStrength {
  let score = 0;
