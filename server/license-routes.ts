...
type LicenseUserRole = "licensor" | "licensee";
type LicenseWithRole = LicenseContract & { userRole: LicenseUserRole };

interface LicenseeMatchUser {
...
function isLicenseeMatch(
  user: LicenseeMatchUser | null | undefined,
...
function getJurisdictionRequirements(code: JurisdictionCode) {
  const template = JURISDICTION_TEMPLATES[code] ?? JURISDICTION_TEMPLATES.US;
  const requirements = template.requirements;

  return {
    template,
    gdprDataProcessingAgreed:
      "gdprDataProcessingAgreed" in requirements ? requirements.gdprDataProcessingAgreed : false,
    gdprWithdrawalRightsAcknowledged:
      "gdprWithdrawalRightsAcknowledged" in requirements
        ? requirements.gdprWithdrawalRightsAcknowledged
        : false,
  };
}
 ...
 */
licenseRouter.get("/", isAuthenticated, async (req: AuthenticatedRequest, res: AppResponse) => {
  try {
    ...
    const licenseeLicenses: LicenseContract[] = [];
    ...
    const licenseMap = new Map<string, LicenseWithRole>();
 ...
 */
licenseRouter.get("/:id", isAuthenticated, async (req: AuthenticatedRequest, res: AppResponse) => {
  try {
    const userId = getUserId(req);
    if (!userId) {
      return res.status(401).json({ error: "Unauthorized" });
    ...
    const isLicensee = isLicenseeMatch(user, license);
...
licenseRouter.post(
  ...
  async (req: AuthenticatedRequest, res: AppResponse) => {
    try {
      ...
      const jurisdictionCode = data.jurisdictionCode ?? "US";
      const {
        template: jurisdictionTemplate,
        gdprDataProcessingAgreed,
        gdprWithdrawalRightsAcknowledged,
      } = getJurisdictionRequirements(jurisdictionCode);
      ...
      const licenseInput: InsertLicenseContract = {
        ...data,
        startsAt,
        expiresAt,
      ...
      const license = await storage.createLicenseContract(licenseInput);
 ...
 */
licenseRouter.post("/:id/sign-licensee", async (req, res: AppResponse) => {
  try {
    const { id } = req.params;
 ...
 */
licenseRouter.post("/:id/verify-payment", async (req, res: AppResponse) => {
  try {
    const { id } = req.params;
 ...
 */
licenseRouter.get("/public/:slug", async (req, res: AppResponse) => {
  try {
  ...
  async (req: AuthenticatedRequest, res: AppResponse) => {
    try {
      ...
      if (license.licenseeWallet !== userWallet) {
        return res
          .status(403)
          .json({ error: "Only the designated licensee can accept this offer" });
      }
 ...
 */
licenseRouter.get("/asset/:assetId", isAuthenticated, async (req, res: AppResponse) => {
  try {
