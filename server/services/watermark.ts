...
export function getFileCategory(
  filename: string
): "code" | "audio" | "image" | "document" | "generic" {
...
export function verifyWatermark(
  ...
  return {
    found: false,
    error: "Use manifest file for non-code files. Provide manifestContent in request.",
  };
}

export function extractWatermarkHash(
  content: string,
  filename: string
): {
  found: boolean;
  ...
  try {
    ...
    if (manifest.signature && manifest.isclId) {
      const watermarkHash = generateWatermarkHash(
        manifest.isclId,
        manifest.ownerWallet,
        manifest.timestamp
      );
