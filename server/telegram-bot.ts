...
class SolturioQuizBot {
    ...
    if (existing.length === 0) {
      ...
      await db
        ...
        .set({
          ...
          dailyCorrectAnswers: shouldResetDaily
            ? isCorrect
              ? 1
              : 0
            : (user.dailyCorrectAnswers || 0) + (isCorrect ? 1 : 0),
  ...
  private async endQuestion(chatId: number) {
    ...
    for (let i = 0; i < correctAnswers.length; i++) {
      ...
      if (rank === 1) {
        ...
        gamePoints = this.calculatePointsByRank(
          rank,
          answer.responseTime,
          this.currentSession.points
        );
    ...
    // Send results
    await this.bot.telegram.sendMessage(chatId, resultsMessage, { parse_mode: "Markdown" });
  ...
  private async showUserStats(ctx: Context) {
    ...
    const user = stats[0];
    const accuracy =
      user.totalQuestions > 0 ? Math.floor((user.correctAnswers / user.totalQuestions) * 100) : 0;
