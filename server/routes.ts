...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype) || allowedExtensions.includes(ext)) {
      ...
      cb(
        new Error(
          `Invalid file type: ${file.mimetype}. Allowed: images, PDF, TXT, ZIP, AI, PSD, EPS, TIFF.`
        )
      );
    }
  },
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch(
    "/api/admin/compliance/triggers/:id",
    isAuthenticated,
    isAdmin,
    async (req: any, res) => {
      try {
  ...
  app.post(
    "/api/admin/compliance/seed-triggers",
    isAuthenticated,
    isAdmin,
    async (req: any, res) => {
      try {
  ...
  app.get("/api/admin/payments", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      if (
        paymentType &&
        ["minting", "rental", "subscription", "iscl"].includes(paymentType as string)
      ) {
      ...
      if (search) {
        conditions.push(
          `(p.transaction_signature ILIKE $${paramIndex} OR p.from_wallet ILIKE $${paramIndex} OR p.to_wallet ILIKE $${paramIndex})`
        );
        params.push(`%${search}%`);
        paramIndex++;
      }
  ...
  app.post("/api/documents/dex-partnership-proposal", async (req, res) => {
    try {
      ...
      const { generateDEXPartnershipProposal } =
        await import("./documents/dex-partnership-proposal");
  ...
  app.post("/api/wallet/create", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { isValidNonce, isValidTimestamp, checkAndStoreNonce } =
        await import("./utils/replay-prevention");
      ...
      const paymentAmountSOL =
        walletType === "standard" ? BigInt(100_000_000) : BigInt(150_000_000); // 0.1 or 0.15 SOL in lamports
