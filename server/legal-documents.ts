 ...
 */
export async function generatePriorArtCertificate(
  ...
  // Footer
  doc.setFontSize(8);
  doc.text('This certificate provides evidence of prior art and first use.', pageWidth / 2, 265, { align: 'center' });
  doc.text('It does not constitute legal advice or formal IP registration.', pageWidth / 2, 270, { align: 'center' });
...
export async function generateCeaseAndDesistLetter(
  ...
  const bodyText = [
    ...
    '',
    'This letter serves as formal notice that you are infringing upon intellectual property rights owned by ' + registration.companyName + '.',
    '',
...
export async function generateEvidencePackage(
  ...
  doc.setFontSize(14);
  doc.text('Intellectual Property First Use Documentation', pageWidth / 2, yPos, { align: 'center' });
  yPos += 20;
  ...
  yPos += 7;
  doc.text(`Prepared: ${new Date().toLocaleDateString()}`, pageWidth / 2, yPos, { align: 'center' });
